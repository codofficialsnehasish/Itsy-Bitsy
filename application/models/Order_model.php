...
class Order_model extends CI_Model
{
    ...
    public function add_order_offline_payment($payment_method,$userId="")
    {
        ...
        $cart_total = $this->cart_model->calculate_cart_total($mode);
        if (!empty($cart_total)) {
            $data = array(
                'order_number' => uniqid(),
                'buyer_id' => 0,
                ...
                'price_gst' => $cart_total->gst,
                'price_shipping' => $cart_total->shipping_cost,
                'discounted_price' => $cart_total->coupone_discount,
                'coupone_code' => $cart_total->coupone_code,
                'price_total' => $cart_total->total,
                'price_currency' => $cart_total->currency,
                'status' => 0,
        	...
        	if($userId!=''){
               ...
               if ($this->db->insert('orders', $data)) {
                ...
                //clear cart
                $this->cart_model->clear_cart();             
                $this->cart_model->clear_coupon_data();             
...
///////update shipping status
	public function updateTrackingNo($order_product_id,$tracking_number,$tracking_url){
     $data = array(
                'order_status' => $this->input->post('order_status', true),
